...
function [words,train_words,target]=Word_freq_my_DB_UTF8_for_NN()
...
for degree=1
    ...
    topic_target{1}='religion';
    topic_target{2}='sport';
    topic_target{3}='history';
    topic_target{4}='technology';
    topic_target{5}='science';
    topic_target{6}='law';
    %topic_target{7}='7.culture\culture (';
    %topic_target{8}='8.philosophy\philosophy (';
    %topic_target{9}='9.geography\geography (';
    %topic_target{10}='10.economy\economy (';
for j1=1:length(topic)
for ii1=1:2:20%2:2:20
clear words;
...
words(:,1)='';
words=words';
train_words{1,ss}=words;
target{1,ss}=topic_target{j1};
ss=ss+1;
end
%msgbox(num2str(toc));
end
